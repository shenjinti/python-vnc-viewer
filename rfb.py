...
class RFBClient(asyncio.Protocol):
    ...
    def sendPassword(self, password):
        """send password"""
        pw = (password + '\0' *
              8)[:8]  # make sure its 8 chars long, zero padded
        des = RFBDes(pw)
    ...
    def vncRequestPassword(self):
        """a password is needed to log on, use sendPassword() to
           send one."""
        if self.getPassword() is None:
            logging.info("need a password\n")
            self.transport.close()
            return
        self.sendPassword(self.getPassword())

    def vncAuthFailed(self, reason):
        """called when the authentication failed.
           the connection is closed."""
        logging.warning("Cannot connect: %s\n" % reason)
